...
class UserManager(BaseUserManager):
    ...
    def create_user(self, username, nama, role, password):
        if not username:
            raise ValueError("Harus ada username!")  # Validasi

        # bikin objek user
        user = self.model(username=username, nama=nama, role=role, password=password)
...
class UserModel(AbstractBaseUser, PermissionsMixin):
    ...
    role = models.CharField(
        max_length=20, choices=Role.choices, default=Role.STAFF, db_column="role"
    )
    ...
    REQUIRED_FIELDS = ["nama", "role", "password"]
...
class StaffModel(models.Model):
    id_staff = models.OneToOneField(
        UserModel, on_delete=models.CASCADE, primary_key=True, db_column="id"
    )
