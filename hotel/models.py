...
class Room(models.Model):
    ...
    room_number = models.CharField(
        max_length=10, unique=True, verbose_name="Nomor Kamar"
    )
    ...
    def is_room_available(room):
        return not Guest.objects.filter(
            room=room, check_out__isnull=True  # if still staying
        )
...
class Guest(models.Model):
    ...
    room = models.ForeignKey(
        Room, on_delete=models.CASCADE, verbose_name="Kamar", db_column="room_id"
    )
    check_in = models.DateTimeField(auto_now_add=True, verbose_name="Waktu Check-In")
    check_in_date = models.DateField()
    check_out = models.DateTimeField(
        null=True, blank=True, verbose_name="Waktu Check-Out", db_index=True
    )
...
class Report(models.Model):
    ...
    report_type = models.CharField(
        max_length=20, choices=ReportType.choices, verbose_name="Jenis Laporan"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["date_from", "date_to"], name="idx_report_period"
            ),  # filter by periode
            models.Index(
                fields=["report_type", "created_at"], name="idx_report_type_date"
            ),  # filter by jenis + tanggal
        ]
